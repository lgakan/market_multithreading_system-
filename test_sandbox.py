...
def sandbox():
    ...
    customer8 = Customer(customer_id=8, shopping_cart=[Item(item_type='Engine', quantity=88), Item(item_type='Wheels', quantity=88)])
    crud.create_customer_in_db(customers = customer8)
    #---------------Create Seller record in database from Seller object-------
    seller7 = Seller(seller_id=7, initial_storage=[Item(item_type="Engine", quantity=18), Item(item_type='Wheels', quantity=11)])
